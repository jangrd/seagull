...
SGL_Element* SGL_ElementNewDebug(const char* file, int line, const char* config, ...) {
    ...
    if (target == NULL) {
        fprintf(stderr, "SGL_ElementNew(): malloc failed\n");
        return NULL;
    }
    // printf("%s\n", line);
    ...
    if (strcmp(config, "") != 0) {
        while (true) {
            ...
            if (value[0] == '\0') {
                ...
                SGL_ElementThrow(target, "Missing value for key (TODO INSERT KEY)");
                break;
            }
            if (strcmp(key, "stack") == 0) {
                ...
                while (*ptr) {
                    if (*ptr < '0' || *ptr > '9') {
                        valid = false;
                        SGL_ElementThrow(target, "Key 'units' expects integer as value");
                        break;
                    }
                    ptr++;
                }
                ...
                if (value[0] != '#') SGL_ElementThrow(target, "Key 'background' expects hex color");
                char* ptr = value + 1;
                ...
                while (*ptr) {
                    if (*ptr >= '0' && *ptr <= '9') {
                        ...
                        SGL_ElementThrow(target, "Key 'background' expects hex color");
                        break;
                    }
                    if ((ptr - value) % 2 == 0) {
                ...
                while (*ptr) {
                    if (*ptr < '0' || *ptr > '9') {
                        valid = false;
                        SGL_ElementThrow(target, "Key 'margin' expects integer as value\n");
                        break;
                    }
                    ptr++;
                }
                ...
                while (*ptr) {
                    if (*ptr < '0' || *ptr > '9') {
                        valid = false;
                        SGL_ElementThrow(target, "Key 'padding' expects integer as value\n");
                        break;
                    }
                    ptr++;
                }
                ...
                while (*ptr) {
                    if (*ptr < '0' || *ptr > '9') {
                        valid = false;
                        SGL_ElementThrow(target, "Key 'gap' expects integer\n");
                        break;
                    }
                    ptr++;
                }
                ...
                while (*ptr) {
                    if (*ptr < '0' || *ptr > '9') {
                        valid = false;
                        SGL_ElementThrow(target, "Key 'opacity' expects integer (0-255)\n");
                        break;
                    }
                    ptr++;
                }
                ...
                SGL_ElementThrow(target, "Unknown key: '%s' (TODO INSERT KEY)\n");
            }
        }
    }
